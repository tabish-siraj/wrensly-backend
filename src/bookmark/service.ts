...
export const CreateBookmark = async (user: any, bookmarkPayload: BookmarkInterface) => {
...
export const CreateDeleteBookmark = async (user: any, bookmarkPayload: BookmarkInterface) => {
    const parsed = BookmarkSchema.safeParse(bookmarkPayload);
    if (!parsed.success) {
        const validationErrors = parsed.error.flatten().fieldErrors;
        logger.warn(`Bookmark validation failed: ${validationErrors}`);
