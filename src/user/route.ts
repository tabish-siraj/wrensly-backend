...
import { PaginationQuerySchema, VerifyEmailSchema } from '../middlewares/validation';

const router = Router();
router.post('/', validateBody(RegisterUserSchema), registerUserController); // Create user
...
router.put(
  ...
  updateUserController
); // Update user by ID (requires verification)
router.get(
  '/suggested',
  authenticateJWT,
