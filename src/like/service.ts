...
export const CreateLike = async (user: any, likePayload: LikeInterface) => {
    ...
    try {
        ...
        if (parsed.data.isLiked) {
            ...
            if (alreadyLiked) {
                logger.warn(`Post with ID ${parsed.data.postId} is already liked by user ${user.id}`);
...
export const DeleteLike = async (user: any, postId: string) => {
    ...
    try {
        ...
        if (!isLiked) {
            ...
            throw new NotFoundError(`Like with Post ID ${postId} not found for user ${user.id}`);
        }
...
export const CreateDeleteLike = async (user: any, likePayload: LikeInterface) => {
    const parsed = LikeSchema.safeParse(likePayload);
    if (!parsed.success) {
        const validationErrors = parsed.error.flatten().fieldErrors;
        logger.warn(`Like validation failed: ${validationErrors}`);
