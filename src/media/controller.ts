  ...
  try {
    ...
    if (!fileName || !fileType || !fileSize || !uploadType) {
      throw new BadRequestError(
        'Missing required parameters: fileName, fileType, fileSize, uploadType'
      );
    }
    ...
    res
      .status(200)
      .json(successResponse('Upload URL generated successfully', result));
  } catch (err) {
    next(err);
  }
};
...
export const confirmUploadController = async (
  ...
  try {
    ...
    if (!uploadId || !fileUrl || !uploadType) {
      throw new BadRequestError(
        'Missing required parameters: uploadId, fileUrl, uploadType'
      );
    }
    ...
    res
      .status(200)
      .json(successResponse('Upload confirmed successfully', null));
  } catch (err) {
    next(err);
  }
};
