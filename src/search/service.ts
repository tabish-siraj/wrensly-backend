...
import {
  calculateRepostData,
  transformPostWithRepostData,
} from '../utils/postTransform';
...
export const searchAll = async (
  ...
  try {
    ...
    const results = await Promise.all([
      type === 'all' || type === 'posts'
        ? searchPosts(user, searchTerm, filters, {
            ...paginationParams,
            limit: 5,
          })
        : { data: [] },
      type === 'all' || type === 'users'
        ? searchUsers(user, searchTerm, filters, {
            ...paginationParams,
            limit: 5,
          })
        : { data: [] },
...
export const searchPosts = async (
  ...
  try {
    ...
    const { repostCountMap, userRepostSet } = await calculateRepostData(
      posts,
      user
    );

    // Transform posts to normalized format with correct repost data
...
export const searchUsers = async (
  ...
  try {
    ...
    const transformedUsers = users.map((foundUser: any) => {
      const counts = followCountMap.get(foundUser.id) || {
        followers: 0,
        following: 0,
      };
