...
export const searchAllController = async (
  ...
  try {
    ...
    if (
      !['all', 'today', 'week', 'month', 'year'].includes(filters.dateRange!)
    ) {
    ...
    res
      .status(200)
      .json(successResponse('Search completed successfully', results));
  } catch (err) {
    next(err);
  }
};
...
export const searchPostsController = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(
        successResponse('Posts search completed', result.data, result.meta)
      );
  } catch (err) {
    next(err);
  }
...
export const searchUsersController = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(
        successResponse('Users search completed', result.data, result.meta)
      );
  } catch (err) {
    next(err);
  }
...
export const searchHashtagsController = async (
  ...
  try {
    ...
    res
      .status(200)
      .json(successResponse('Hashtags search completed', hashtags));
  } catch (err) {
    next(err);
  }
};
...
export const getSearchSuggestionsController = async (
  ...
  if (!req.user) {
    throw new UnauthorizedError(
      'You must be logged in to get search suggestions'
    );
  }
  ...
  try {
    ...
    if (q.trim().length < 2) {
      return res.status(200).json(
        successResponse('Search suggestions', {
          users: [],
          hashtags: [],
          recent_searches: [],
        })
    ...
    res
      .status(200)
      .json(successResponse('Search suggestions retrieved', suggestions));
  } catch (err) {
    next(err);
  }
};
