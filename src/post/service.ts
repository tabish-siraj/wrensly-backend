...
export const CreateRepost = async (
  ...
  try {
    ...
    if (parentPost.type !== 'POST' && parentPost.type !== 'QUOTE')
      throw new BadRequestError('Only posts and quotes can be reposted');
  ...
  return posts.map((post) => {
    if (post.type === 'REPOST' && post.parent) {
      ...
      return {
        ...
        hashtags:
          originalPost.hashtags?.map((ph: any) => ph.hashtag.name) || [],
...
export const DeletePost = async (user: UserPayload, post_id: string) => {
  try {
    ...
    if (post.userId !== user.id) {
      logger.warn(
        `User ${user.id} attempted to delete post ${post_id} owned by ${post.userId}`
      );
    ...
    logger.info(
      `Post ${post_id} deleted by user ${user.id}. Had ${post._count.children} comments, ${post._count.likes} likes, ${post._count.bookmarks} bookmarks`
    );
